...
This engine also assumes that your layout will handle the flash notifications and are loading both jquery and jquery_ujs in the header BEFORE the yield tag above.
...
===== example
  ...
  class ApplicationController < ActionController::Base
    before_filter :prevent_unauthorized_reports
    def prevent_unauthorized_reports
      if params[:controller] =~ /i_wonder/
...
You can choose to navigate directly to the dashboard. If you want to include a link on your admin page to the i_wonder dashboard, your can access the url with <code>i_wonder.root_url</code> such as:
  ...
  <%= link_to "Go to analytics >", i_wonder_path %>
...
If you want to configure this gem, add this to an initializer.
  ...
  IWonder.configuration do |c|
    ...
    c.only_log_hits_on_200 = false   # By default hits and new visitors won't be logged on anything but a 200 or 304 (not modified). Your custom events will still be recorded.
  end
